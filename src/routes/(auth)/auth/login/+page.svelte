...
<Card.Root>
	...
	<Card.Content>
		<Form.Root method="POST" action="?/loginUser" options={{
			flashMessage: {
				module: flashModule,
				onError: ({message}) => {
			...
			delayMs: 500,
			timeoutMs: 8000,
		}} {form} schema={loginUserSchema} let:config>
		...
		<div class="flex flex-col gap-2">
			<a href="/password-reset" class="text-xs font-medium  text-right self-end max-w-max text-primary hover:underline">
	...
	<div class="flex flex-col mt-12 space-y-4 social-logins">
		<Button href="/api/oauth/github" variant="outline">Login with GitHub</Button>
		<Button href="/api/oauth/google" variant="outline">Login with Google</Button>
	</div>
</Card.Content>
</Card.Root>
